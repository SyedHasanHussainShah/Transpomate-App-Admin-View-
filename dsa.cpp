...
vector<string> predefinedStops = {
    // Gujranwala Stops
    "Gujranwala City", "Gujranwala Railway Station", "Faisal Town", "GT Road",

    // Islamabad Stops
    "Blue Area", "F-6 Markaz", "D-12", "Sector G-11", "Islamabad",

    // Lahore Stops
    "Lahore Fort", "Badshahi Mosque", "Mall Road", "Liberty Market", "DHA Phase 5", "Gulberg"};

// Predefined routes with distances (in km) and times (in minutes)
map<pair<string, string>, pair<int, int>> predefinedRoutes = {
    {{"Gujranwala City", "Islamabad"}, {5, 10}},
    {{"Gujranwala City", "Faisal Town"}, {7, 15}},
    {{"Islamabad", "GT Road"}, {3, 8}},
    {{"Blue Area", "F-6 Markaz"}, {4, 12}},
    {{"F-6 Markaz", "D-12"}, {8, 18}},
    {{"Sector G-11", "Islamabad"}, {6, 14}},
    {{"Lahore Fort", "Badshahi Mosque"}, {2, 5}},
    {{"Mall Road", "Liberty Market"}, {6, 15}},
    {{"DHA Phase 5", "Gulberg"}, {10, 20}},
    {{"Gujranwala City", "Blue Area"}, {200, 180}},
    {{"Blue Area", "Lahore Fort"}, {290, 240}},
    {{"Gujranwala City", "Lahore Fort"}, {70, 90}}};
...
void Bus::installBus() {
    ...
    cout << "                         | Arrival time (e.g., 10:00 AM): ";
    getline(cin, bus[p].arrival);

    cout << "                         | Departure time (e.g., 5:00 PM): ";
    getline(cin, bus[p].depart);

    cout << "                         | From (start location): ";
    getline(cin, bus[p].from);

    cout << "                         | To (end location): ";
...
void Bus::availableBuses() {
    ...
    while (getline(busFile, busNumber)) {
        ...
        // If a bus record is valid, set the busExists flag
        busExists = true;

        // Display the bus details in a formatted way
        cout << "                         | " << busNumber << "\n";
        cout << "                         | " << driver << "\n";
        cout << "                         | " << arrival << "\n";
        cout << "                         | " << depart << "\n";
...
void generateMapURL(const vector<pair<string, string>>& predefinedRoutes) {
    for (const auto& route : predefinedRoutes) {
        ...
        // URL encode the stop and destination
        string encodedStop = urlEncode(stop);
        string encodedDestination = urlEncode(destination);

        // Construct the URL dynamically for each route
        string url = "https://www.openstreetmap.org/directions?from=" + encodedStop + "&to=" + encodedDestination;

        // Open the map in the browser for each route
        openInBrowser(url);
    }
}
...
void Bus::addRoute() {
    ...
    string stopChoice, destinationChoice;
...
void Bus::editRoute() {
    ...
    // Ask if the user wants to see the updated map
    cout << "\nWould you like to see the map for the updated routes? (y/n): ";
    char mapChoice;
    cin >> mapChoice;
...
void Bus::removeRoute() {
    ...
    // Optionally, generate a map for the remaining routes
    cout << "\nWould you like to see the map for the remaining routes? (y/n): ";
    char mapChoice;
    cin >> mapChoice;
    cin.ignore(); // Clear the newline character from the input buffer
...
void Bus::displayFeedback()
{
    ...
    if (feedbacks.empty())
    {
        ...
        if (feedbackFile.is_open())
        {
            ...
            {
                if (line.find("Bus Number: ") == 0)
                { // If it starts with "Bus Number:"
